...
typedef struct {
      int jhbitlen;	                /*the message digest size*/
...
inline static void jhF8(jhState *state);    /* the compression function F8 */

/*The API functions*/
jhReturn jhInit(jhState *state, int jhbitlen);
jhReturn jhUpdate(jhState *state, const BitSequence *data, DataLength databitlen);
jhReturn jhFinal(jhState *state, BitSequence *jhval);
jhReturn jhHash(int jhbitlen, const BitSequence *data,DataLength databitlen, BitSequence *jhval);
...
#define round_function(nn,r)                                                              \
      jhSS(y0,y2,y4,y6,y1,y3,y5,y7, LOAD(jhE8_bitslice_roundconstant[r]), LOAD(jhE8_bitslice_roundconstant[r]+16) ); \
      jhlineartransform_R##nn(y0,y2,y4,y6,y1,y3,y5,y7);

/*the compression function F8 */
inline static void jhF8(jhState *state)
{
      uint64 i;
...
jhReturn jhFinal(jhState *state, BitSequence *jhval)
{
      ...
      switch (state->jhbitlen)
      {
          case 224: memcpy(jhval,t+36,28); break;
          case 256: memcpy(jhval,t+32,32); break;
          case 384: memcpy(jhval,t+16,48); break;
          case 512: memcpy(jhval,t,64);    break;
      }
...
jhReturn jhHash(int jhbitlen, const BitSequence *data,DataLength databitlen, BitSequence *jhval)
{
      ...
      if ( jhbitlen == 224 || jhbitlen == 256 || jhbitlen == 384 || jhbitlen == 512 )
      {
          jhInit(&state, jhbitlen);
          jhUpdate(&state, data, databitlen);
          jhFinal(&state, jhval);
          return jhSUCCESS;
      }
      else
          return(jhBAD_HASHLEN);
}
